...
"""
Programme that takes the DNA sequences as an input from a single external
file and saves the best alignment(s) along with its corresponding score in a
single text file - but this includes partial overlap of strands at both ends,
not just latter
"""
...
def main(argv):
    ...
    if len(sys.argv) == 3 and sys.argv[1].endswith("fasta") == \
    True and sys.argv[2].endswith("fasta") == True:
        ...
        seq1, seq2 = get_seq("../data/407228326.fasta"),\
             get_seq("../data/407228412.fasta")
    ...
    for i in range(l1 + l2 -1):
        ...
        statement = "This alignment occurs when the smaller strand (" + \
            ...
            " of the larger strand, with the highest score of " + \
            str(score) + ":" + "\n"
        #statement explaining the alignment in detail
        best_comparison_highSP, best_comparison_lowSP, best_s2, best_s1 = \
            (shift + matched + (l2 - 1) * "." + "\n"), \
        ...
        # giving its score and startpoint, and assigns the 3 lines of alignment 
